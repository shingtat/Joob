			...
			if (xhttp.responseText.localeCompare("false")!=0)
				{
					...
					for (var i = 0; i < arr.length; i ++) {
						...
						{
							str = str + '<p "id = left">'+line[0]+'</p>'+ "<br>";
						}
						else
						{
							str = str + '<p "id = right">'+line[0]+'</p>' + "<br>";
						}
					}
				}
...
function saveName(username,type){
	var user = username;
	var xhttp = new XMLHttpRequest();
	xhttp.open("GET", window.location.pathname.substring(0, window.location.pathname.indexOf("/", 1)) +"/ChatServer?"+"savedName"+"="+user, false);
	xhttp.send();
